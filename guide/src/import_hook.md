...
automatically like edits to Python components do. Without rebuilding, Python code would run but use outdated Rust
components, leading to confusing behaviour.

The import hook can be installed into a virtual environment or activated manually by individual scripts.
...
The hook has some additional features such as importing standalone `.rs` files.
...
```
This will activate the hook automatically when the interpreter starts.
...
To avoid unnecessary rebuilds caused by irrelevant file modifications, add empty `.maturin_hook_ignore` files to
...
* Importing standalone `.rs` files that use `PyO3` bindings (see [Import Rust File](#import-rust-file)).
* `importlib.reload()` (currently unsupported on Windows).
* Detecting edits to path dependencies at any depth.
* Multiple environments with different interpreters as each environment has a separate build cache.
...
This is another reason why installing site-wide is convenient.

The installation works across multiple Python modules (e.g. can just put `install()` at the top of the main module)
but the hook will then not be active when running/importing another module directly, for example in tests.
`install()` can be called in each module but be aware that each call replace the previous settings.
...
```
Then call `install_maturin_hook()` in each module instead. This will ensure the custom options are used everywhere
...
returns `None`. This mechanism can be used to create modules on demand (e.g. the `.rs` importer) or trigger
...
If [sitecustomize.py](https://docs.python.org/3/library/site.html) exists in the `site-packages` directory of a Python
installation it is loaded automatically by the Python interpreter unless the `-S` flag is passed to it. This makes it a
convenient place to activate the import hook.
